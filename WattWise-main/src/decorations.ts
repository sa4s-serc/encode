...
export class EnergyDecorationManager {
    ...
    private createHoverContent(prediction: EnergyPrediction): vscode.MarkdownString {
        ...
        markdown.appendMarkdown(`### ${tierEmoji} WattWise — Energy Estimate\n\n`);
        ...
        // AI suggestion section (High-tier blocks only)
        if (prediction.energy_tier === 'High' && prediction.suggestion) {
            const s = prediction.suggestion;
            if (s.error) {
                markdown.appendMarkdown(`---\n\n`);
                markdown.appendMarkdown(`🤖 **AI Suggestion unavailable:** ${s.error}\n\n`);
            } else {
                markdown.appendMarkdown(`---\n\n`);
                markdown.appendMarkdown(`### 🤖 Gemini 2.5 Flash Suggestion\n\n`);
                if (s.explanation) {
                    markdown.appendMarkdown(`**Why it's expensive:** ${s.explanation}\n\n`);
                }
                if (s.suggestions && s.suggestions.length > 0) {
                    markdown.appendMarkdown(`**Improvements:**\n\n`);
                    s.suggestions.forEach(tip => {
                        markdown.appendMarkdown(`- ${tip}\n`);
                    });
                    markdown.appendMarkdown(`\n`);
                }
                if (s.improved_code) {
                    markdown.appendMarkdown(`**Suggested code:**\n\n`);
                    markdown.appendCodeblock(s.improved_code, 'python');
                    markdown.appendMarkdown(`\n`);
                }
            }
        }
